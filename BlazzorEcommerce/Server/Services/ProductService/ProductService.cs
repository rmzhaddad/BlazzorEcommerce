...
namespace BlazzorEcommerce.Server.Services.ProductService
{
    public class ProductService : IProductService
    {
        ...
        public async Task<ServiceResponse<List<Product>>> GetFeaturedProducts()
        {
            ...
            {
                Data= await _context.Products.Where(p=>p.Featured==true).Include(p=>p.Variants).ToListAsync()
            };
            return response;
        }
        ...
        public async Task<ServiceResponse<ProductSearchResult>> SearchProducts(string searchText, int page)
        {
            ...
            var products = await _context.Products.Where(p => p.Title.ToLower().Contains(searchText.ToLower())
                            || p.Description.ToLower().Contains(searchText.ToLower())).Include(p => p.Variants)
